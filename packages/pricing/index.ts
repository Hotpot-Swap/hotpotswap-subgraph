import {
  ...
  BIG_DECIMAL_ZERO,
  FACTORY_ADDRESS,
  HOTPOTSWAP_WETH_USDT_PAIR_ADDRESS,
  HOTPOT_TOKEN_ADDRESS,
  HOTPOT_USDT_PAIR_ADDRESS,
  UNISWAP_FACTORY_ADDRESS,
  UNISWAP_HOTPOT_ETH_PAIR_FIRST_LIQUDITY_BLOCK,
  UNISWAP_HOTPOT_USDT_PAIR_ADDRESS,
...
export function getHotpotPrice(block: ethereum.Block): BigDecimal {
  if (block.number.lt(UNISWAP_HOTPOT_ETH_PAIR_FIRST_LIQUDITY_BLOCK)) {
    // If before uniswap hotpot-eth pair creation and liquidity added, return zero
    return BIG_DECIMAL_ZERO;
  } else if (block.number.lt(BigInt.fromI32(10800029))) {
    // Else if before uniswap hotpot-usdt pair creation (get price from eth hotpot-eth pair above)
    return getUSDRate(HOTPOT_TOKEN_ADDRESS, block);
  } else {
    // Else get price from either uni or hotpot usdt pair depending on space-time
