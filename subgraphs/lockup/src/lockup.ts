...
import { getHotpotPrice } from 'pricing'
...
export function getUser(pid: BigInt, address: Address, block: ethereum.Block): User {
  ...
  if (user === null) {
    ...
    user.hotpotHarvestedSinceLockup = BIG_DECIMAL_ZERO
    user.hotpotHarvestedSinceLockupUSD = BIG_DECIMAL_ZERO
    user.save()
  }
...
function transfer(pid: BigInt, userAddr: Address, block: ethereum.Block): void {
  ...
  if (block.number.ge(LOCKUP_BLOCK_NUMBER)) {
    ...
    if (pending.gt(BIG_DECIMAL_ZERO)) {
      user.hotpotHarvestedSinceLockup = user.hotpotHarvestedSinceLockup.plus(pending)
      const hotpotHarvestedUSD = pending.times(getHotpotPrice(block))
      user.hotpotHarvestedSinceLockupUSD = user.hotpotHarvestedSinceLockupUSD.plus(hotpotHarvestedUSD)
    }
  }
