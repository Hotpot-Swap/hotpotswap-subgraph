...
import { getHotpotPrice, getUSDRate } from 'pricing'
...
function getMasterChef(block: ethereum.Block): MasterChef {
  ...
  if (masterChef === null) {
    ...
    masterChef.hotpot = contract.hotpot()
    masterChef.hotpotPerBlock = contract.hotpotPerBlock()
...
export function getPool(id: BigInt, block: ethereum.Block): Pool {
  ...
  if (pool === null) {
    ...
    pool.accHotpotPerShare = poolInfo.value3
    ...
    pool.hotpotHarvested = BIG_DECIMAL_ZERO
    pool.hotpotHarvestedUSD = BIG_DECIMAL_ZERO
    pool.save()
  }
...
export function deposit(event: Deposit): void {
  ...
  // Calculate HOTPOT being paid out
  if (event.block.number.gt(MASTER_CHEF_START_BLOCK) && user.amount.gt(BIG_INT_ZERO)) {
    const pending = user.amount
      .toBigDecimal()
      .times(pool.accHotpotPerShare.toBigDecimal())
      .div(BIG_DECIMAL_1E12)
      .minus(user.rewardDebt.toBigDecimal())
      .div(BIG_DECIMAL_1E18)
    // log.info('Deposit: User amount is more than zero, we should harvest {} hotpot', [pending.toString()])
    if (pending.gt(BIG_DECIMAL_ZERO)) {
      // log.info('Harvesting {} HOTPOT', [pending.toString()])
      const hotpotHarvestedUSD = pending.times(getHotpotPrice(event.block))
      user.hotpotHarvested = user.hotpotHarvested.plus(pending)
      user.hotpotHarvestedUSD = user.hotpotHarvestedUSD.plus(hotpotHarvestedUSD)
      pool.hotpotHarvested = pool.hotpotHarvested.plus(pending)
      pool.hotpotHarvestedUSD = pool.hotpotHarvestedUSD.plus(hotpotHarvestedUSD)
      poolHistory.hotpotHarvested = pool.hotpotHarvested
      poolHistory.hotpotHarvestedUSD = pool.hotpotHarvestedUSD
    }
  }
...
export function withdraw(event: Withdraw): void {
  ...
  if (event.block.number.gt(MASTER_CHEF_START_BLOCK) && user.amount.gt(BIG_INT_ZERO)) {
    const pending = user.amount
      ...
      .minus(user.rewardDebt.toBigDecimal())
      .div(BIG_DECIMAL_1E18)
    // log.info('Withdraw: User amount is more than zero, we should harvest {} hotpot - block: {}', [
    //   pending.toString(),
    //   event.block.number.toString(),
    // ])
    // log.info('HOTPOT PRICE {}', [getHotpotPrice(event.block).toString()])
    if (pending.gt(BIG_DECIMAL_ZERO)) {
      // log.info('Harvesting {} HOTPOT (CURRENT HOTPOT PRICE {})', [
      //   pending.toString(),
      //   getHotpotPrice(event.block).toString(),
      // ])
      const hotpotHarvestedUSD = pending.times(getHotpotPrice(event.block))
      user.hotpotHarvested = user.hotpotHarvested.plus(pending)
      user.hotpotHarvestedUSD = user.hotpotHarvestedUSD.plus(hotpotHarvestedUSD)
