...
export function queuedTransaction(event: QueueTransaction): void {
  const GRACE_PERIOD = 14 // expiry period (days) - https://github.com/hotpotswap/hotpotswap/blob/26af8ce6d573346d06d254fe83248553523e1f96/contracts/Timelock.sol#L27
  let id = event.params.txHash.toHexString()
  let tx = Timelock.load(id)
  if (tx === null) {
    tx = new Timelock(id)
