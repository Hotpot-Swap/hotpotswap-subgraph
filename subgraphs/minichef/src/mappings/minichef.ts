import { ACC_HOTPOT_PRECISION, ADDRESS_ZERO, BIG_INT_ONE, BIG_INT_ZERO } from 'const'
import {
  Deposit,
  EmergencyWithdraw,
  Harvest,
...
export function logUpdatePool(event: LogUpdatePool): void {
  log.info('[MiniChef] Log Update Pool {} {} {} {}', [
    ...
    event.params.accHotpotPerShare.toString(),
  ])
  const pool = getPool(event.params.pid, event.block)
  pool.accHotpotPerShare = event.params.accHotpotPerShare
  pool.lastRewardTime = event.params.lastRewardTime
  pool.save()
}

export function logHotpotPerSecond(event: LogHotpotPerSecond): void {
  log.info('[MiniChef] Log Hotpot Per Second {}', [event.params.hotpotPerSecond.toString()])

  const miniChef = getMiniChef(event.block)

  miniChef.hotpotPerSecond = event.params.hotpotPerSecond
  miniChef.save()
}
...
export function deposit(event: Deposit): void {
  ...
  user.rewardDebt = user.rewardDebt.plus(event.params.amount.times(pool.accHotpotPerShare).div(ACC_HOTPOT_PRECISION))
  user.save()
}
...
export function withdraw(event: Withdraw): void {
  ...
  user.rewardDebt = user.rewardDebt.minus(event.params.amount.times(pool.accHotpotPerShare).div(ACC_HOTPOT_PRECISION))
  user.save()
}
...
export function harvest(event: Harvest): void {
  ...
  const accumulatedHotpot = user.amount.times(pool.accHotpotPerShare).div(ACC_HOTPOT_PRECISION)

  user.rewardDebt = accumulatedHotpot
  user.hotpotHarvested = user.hotpotHarvested.plus(event.params.amount)
  user.save()
}
